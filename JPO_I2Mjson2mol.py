...
#NOTE #change this arg pretrained in configs/rtetr/include/rtdetr_r500vd.yaml to avoid download the pretrained model from github, as timeout error
#or change the code of presnet.py
_model=cfg.model
...
print(f' {bcolors.WARNING} loaded keys {i}, skip mismatch size keys {j} (same nnName but diff dimensions) {bcolors.ENDC}')


postprocessor = cfg.postprocessor##RTDETRPostProcessor@@src/zoo/rtertr
# postprocessor2=RTDETRPostProcessor(num_classes=24, use_focal_loss=True, num_top_queries=300, remap_mscoco_category=False)
...
for ff in pngs:
    ...
    if getacc:
        ...
        if (type(SMILESori)!=type('a')):
            sameWithOutStero=True# the original csv file do not have the ref SMILES
            ...
            #only use the longsest to comparing
        ...
        if sameWithOutStero or sameWithOutStero_exp or logest_flag or logest_flagHfix:
            ...
            if rt_out and has_ref:
                ...
                # Resolve paths to absolute to compare
                ...
                if abs_image_path != abs_ac_datadir_img:
                    ...
                    # print(f"Skipping copy: {image_path} and {ac_datadir_img} are the same file")
                    pass 
                ...
                # ac_datadirD=  os.path.join(view_dirac2, f"{file_base}.json")
                # b_datadirD=  os.path.join(view_dirb2, f"{file_base}.json")
                # b_datadir_imgD=  os.path.join(view_dirb2, f"{file_base}.png")
                # os.remove(b_datadir_imgD)
                # os.remove(ac_datadirD)
                # os.remove(b_datadirD)
...
# print(f"miss files number {len(miss_file)}")
...
#TODO add output the codes fixed box or shapes!!

#    nohup python I2M0411.py > USPTO_0411.log  2>&1 &
#  nohup python I2Mjson2mol.py > staker_0424I2Mjson2mol.log  2>&1 &